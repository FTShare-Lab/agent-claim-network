    ...
    endpoints: [
      {
        ...
        summary: 'Sign in and receive an expiring HttpOnly Cookie.',
        response: '{ id, username, expires_at } and Set-Cookie',
        caller: 'Workbench sign-in page',
        body: 'HTTP Basic credentials and X-ACN-Workbench: 1. Response contains session metadata.',
      },
      {
        method: 'POST',
        path: '/api/admin-auth/logout',
        summary: 'Revoke the current Workbench session and delete its Cookie.',
        response: '204 No Content and expired Set-Cookie',
        caller: 'Workbench sign-out button',
        body: 'Cookie, X-ACN-Workbench: 1 and X-ACN-Admin-Session headers.',
      },
      {
        method: 'POST',
        ...
        response: '204 No Content (valid credentials, empty body)\n401 Unauthorized (incorrect username or password)',
        caller: 'Basic Auth clients',
      },
      {
        method: 'GET',
        path: '/api/admin-auth/status',
        summary: 'Check administrator authentication and restore a valid Cookie session',
        response: '{ enabled: boolean, session?: { id, username, expires_at } }',
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/health',
        summary: 'Public liveness and team authentication status',
        response: '200 OK: { status: "ok", team_auth_enabled: boolean }',
        caller: 'Orchestration / health checks',
      },
      {
        method: 'POST',
        path: '/health',
        summary: 'Probe team credentials without changing the HTTP health status',
        body: '{ "auth": { "agent_id": "agent-a", "acn_key": "<team-key>" }, "data": {} }',
        response: '200 OK: { status: "ok", team_auth_enabled: boolean, auth_passed: boolean }. Auth disabled bypasses verification. Malformed JSON/envelopes return 400.',
        caller: 'Agent connectivity diagnostics; Router exposes the same endpoint',
      },
      {
        method: 'GET',
        path: '/status',
