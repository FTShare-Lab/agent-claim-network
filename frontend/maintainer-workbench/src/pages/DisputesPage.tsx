...
function DirectClaimCard({ claim, onOpen }: { claim: Claim; onOpen: () => void }) {
  return (
    <article aria-label={claim.name} className="rounded-md border border-slate-200 bg-white p-3">
      ...
      <div className="mt-3">
        ...
        <p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-slate-700">{claim.statement}</p>
      </div>
      <div className="mt-3 border-t border-slate-100 pt-3">
        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Evidence Summary</div>
        <p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-slate-600">{claim.evidence_summary}</p>
      </div>
      <div className="mt-3 border-t border-slate-100 pt-3">
          ...
          selectedDispute ? (
            <div className="space-y-2">
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  type="button"
                  className="rounded-md border border-blue-200 px-3 py-1.5 text-sm font-medium text-blue-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400"
                  disabled={isStaticDemo || selectedDispute.status !== 'open' || analyzeMutation.isPending}
        ...
        {selectedDispute ? (
          <>
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              ...
              <div aria-label="Direct claim details" className="mt-2 space-y-2">
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Automatic Analysis</div>
              {analyses.isLoading && !automaticAnalysis ? <div className="mt-2 text-xs text-slate-500">Loading automatic analysis…</div> : null}
              {analyses.error ? <div className="mt-2 text-xs text-rose-700">{String(analyses.error)}</div> : null}
              {automaticAnalysisDetail.error ? <div className="mt-2 text-xs text-rose-700">{String(automaticAnalysisDetail.error)}</div> : null}
              <div className="mt-2">
                {automaticAnalysis ? (
                  <AnalysisCard
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Manual Analysis</div>
              {manualAnalysisDetail.error ? <div className="mt-2 text-xs text-rose-700">{String(manualAnalysisDetail.error)}</div> : null}
              <div className="mt-2">
                {manualAnalysis ? (
                  <AnalysisCard
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Current Resolution</div>
              {selectedDispute.resolution ? (
                <ResolutionDetail resolution={selectedDispute.resolution} />
              ) : (
                <div className="mt-2 text-xs text-slate-500">No resolution has closed this dispute.</div>
              )}
            </div>
          ...
          <>
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              ...
              {selectedClaim.claim.source_claim_ids.length ? (
                ...
                <div className="mt-2 text-xs text-slate-500">No sources</div>
              )}
            </div>
          </>
        ) : null}
