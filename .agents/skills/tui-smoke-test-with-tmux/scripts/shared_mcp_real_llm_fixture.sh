#!/usr/bin/env bash
# 供 tui_tmux_shared_mcp_real_llm.sh 使用的协议真实 stdio MCP fixture。
set -euo pipefail
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"tools/call"'*)
      ...
      progress_token="$(printf '%s' "$line" | sed -n 's/.*"progressToken":"\([^"]*\)".*/\1/p')"
      respond_tool "$request_id" "$tool" "$progress_token" &
      ;;
