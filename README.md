...
- **Maintainer**：收 Claim 镜像与 dispute，发 policy，做过期扫描，往各 Agent 的 inbox 投递建议；还可选用独立 LLM 分析冲突。每个 dispute 只有一个 Current Analysis：`manual` 只响应管理者显式 Analyze，`shadow`/`auto` 在上报时创建分析，且只有创建于 `auto`、当前仍运行在 `auto` 的分析自动采用。普通建议、自动 Resolution、人工 Resolve 与 Reject & Replace 都通过同一种 ClaimAttributeUpdate 交给 holder；Agent 可更新全部非 deprecated 本地 Claim和自己持有的任意状态 direct Claim，其他 holder 快照只读，每条消息由 Effect Journal 负责崩溃恢复。
