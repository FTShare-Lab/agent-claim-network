...
复制 [automated-run.example.json](manifests/automated-run.example.json) 到仓库外的绝对路径，并填写本机 checkout、`acn_eval` 和 run root。配置中禁止 key；外部启动器必须沿用已有的 `ACN_EVAL_UPSTREAM_KEY`，同时提供 `ACN_EVAL_UPSTREAM_BASE_URL`。`model` 是路由别名 `deepseek-v4-flash-local-exp`，`response_model` 是上游实际回显的 `deepseek-v4-flash`。

30 个 `task_workers` 配合示例中每题 2 CPU / 8 GiB，真实执行前会要求 Docker 至少提供 60 CPU 和 240 GiB 内存。资源不够会直接退出，不会降低并发或启动部分任务。
