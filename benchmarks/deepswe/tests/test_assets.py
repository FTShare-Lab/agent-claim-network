...
class AssetTests(unittest.TestCase):
    def test_coding_benchmark_skill_is_hashed_stable_and_contains_no_task_answer(self) -> None:
        ...
        self.assertIn("验收条件", content)
        self.assertIn("git commit", content)
        self.assertIn("本地套件", content)
        self.assertIn("完整项目测试", content)
        self.assertIn("/sys/fs/cgroup/pids.max", content)
        self.assertIn("降低 worker 数", content)
