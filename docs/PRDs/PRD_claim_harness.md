...
- `src/tool/claim.rs`、registry 与 bootstrap：注册并装配普通主 agent 专用工具；subagent 与 memory review profile 不获得该权限。
- `src/agent/session_engine/prompts.rs`、`prompts/agent_system.j2`：普通会话用轻目录，说明检索、证据核对和修订规则。
