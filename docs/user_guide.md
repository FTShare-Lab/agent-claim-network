...
Agent 对普通建议、自动 Resolution、人工 Resolve 与 Reject & Replace 使用同一条 ClaimAttributeUpdate 内化流程。每次调用只处理一条消息，输入完整 CAU、规范化 conclusion、可选 Resolution/Dispute、全部非 deprecated 本地 Claim、当前 holder 持有的任意状态 direct Claim，以及其他 holder 的只读 direct Claim 快照；不读取 Memory 或 USER。当前 Agent 可以原地更新输入中的任意本地 Claim，但非 direct deprecated Claim 不会进入输入，其他 holder 的 Claim 不能修改。已经符合结论或无需修改时可以保持不变；存在 Resolution 时优先复用正确知识和原地修正同一知识单元，避免为观察效果制造重复 Claim。每条 CAU 的 Effect Journal 保存已校验效果，崩溃恢复不再次调用模型。

Policy 内化若已经通过更正或 deprecated Claim 消除冲突，就不会再上报对应 Dispute。Agent 会在写文件和上传前依据整批内化后的最终 Claim 状态校验这一点；Maintainer 也会拒绝 direct Claims 中包含 deprecated Claim 的首次新上报。这是确定性冲突，Agent 不自动重试。已经存在且原始内容相同的 Dispute 网络重放仍保持幂等。
