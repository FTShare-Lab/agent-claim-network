...
impl ClaimSummary {
    pub fn from_claim(claim: &Claim) -> Self {
        Self {
            ...
            status: claim.status,
            updated_at: claim.effective_updated_at(),
        }
    }
}
...
pub(crate) mod tests {
    ...
    #[tokio::test]
    async fn pending_edit_recovery_yields_to_same_second_local_write() {
        let dir = TempDir::new().unwrap();
        let runner = team_runner(&dir);
        let original = sample_claim(ClaimId::random(), runner.agent_id().clone());
        let mut target = original.clone();
        target.name = "target".into();
        target.updated_at = Some(original.created_at + Duration::seconds(1));
        // 另一次持锁写入落在同一秒：内容不同，updated_at 不晚于 target。
        let mut competing = original.clone();
        competing.status = ClaimStatus::Deprecated;
        competing.updated_at = target.updated_at;
        runner.claim_store.write_claim(&competing).await.unwrap();
        write_yaml_atomic(
            &paths::agent_home_claim_edit_pending_path(dir.path()),
            &PendingClaimEdit {
                expected_revision: claim_revision(&original).unwrap(),
        ...
        .await
        .unwrap();
        assert_eq!(queued.claims, vec![competing]);
        assert!(
            !tokio::fs::try_exists(paths::agent_home_claim_edit_pending_path(dir.path()))
                .await
                .unwrap()
