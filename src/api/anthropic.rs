...
/// 请求携带媒体块时，上游明确的请求格式错误可能表示模型不支持多模态：
...
fn wrap_media_rejection(error: AnthropicError, request_has_media: bool) -> AnthropicError {
    ...
    match error {
            ...
            if crate::api::is_provider_request_error(status, &body) =>
        {
            ...
            let body = redact_anthropic_error_body(&body);
            if content_policy {
                AnthropicError::Status { status, body }
            } else {
                AnthropicError::MediaRejected {
                    source: Box::new(AnthropicError::Status { status, body }),
                }
            }
        }
        other => other,
...
fn classified_anthropic_error_type(body: &str) -> Option<String> {
    let structured_type = serde_json::from_str::<Value>(body).ok().and_then(|value| {
        value
            .get("error")
...
mod tests {
    ...
    #[test]
    fn media_request_4xx_wraps_hint_without_echoing_upstream_body() {
