...
pub enum McpOAuthError {
    ...
    #[error("MCP server '{server}' 需要 OAuth 登录；请执行 `acn mcp login {server}`")]
    AuthorizationRequired { server: String },
    #[error("MCP server '{server}' 的已保存 OAuth client 与当前 oauth_client_id 不匹配；请重新执行 `acn mcp login {server}`")]
    ClientIdMismatch { server: String },
...
pub(crate) struct McpRuntimeAuthorization {
    ...
    pub(crate) expected_client_id: Option<String>,
}
...
impl CredentialStore for McpCredentialStore {
    ...
    async fn clear(&self) -> Result<(), AuthError> {
        // rmcp 在 issuer 变化等身份失效场景调用 clear。这里只能丢弃旧 token；保留
        // enrollment marker，确保没有显式 OAuth 选项的已登录 server 下次启动仍
        // fail closed。只有用户显式 logout/remove 才删除登记。
...
pub async fn login(
    ...
    let redirect_url = format!("http://127.0.0.1:{callback_port}{LOGIN_CALLBACK_PATH}");
    ...
    let callback_result = if no_browser {
        ...
        println!(
            "请在本地浏览器打开以下 URL 完成 MCP server '{server_name}' 的登录：\n\
             {authorization_url}\n\
             浏览器跳转到 127.0.0.1 后，无需等待页面加载；\n\
             请复制地址栏中的完整 URL 并粘贴到这里。"
        );
        let callback_url = read_callback_url(server_name).await?;
        validate_pasted_callback_url(server_name, &callback_url, &redirect_url)?;
...
pub(crate) async fn authorization_manager(
    ...
    let result = async {
        ...
        authorization_manager_locked(server_name, url, credentials.clone(), expected_client_id)
            .await
    }
    .await;
    credentials.clear_mutation_lease();
...
pub(crate) async fn authorization_manager_locked(
    ...
    if expected_client_id.is_some_and(|expected| expected != stored.client_id) {
        return Err(McpOAuthError::ClientIdMismatch {
            server: server_name.to_string(),
        });
    }
    validate_oauth_resource_url(server_name, url)?;
    ...
    Ok(Some(McpRuntimeAuthorization {
        ...
        expected_client_id: expected_client_id.map(str::to_string),
    }))
}
...
fn validate_pasted_callback_url(
    server_name: &str,
    value: &str,
    expected: &str,
) -> Result<(), McpOAuthError> {
    let invalid = || McpOAuthError::CallbackInput {
        server: server_name.to_string(),
    };
    let value = reqwest_013::Url::parse(value).map_err(|_| invalid())?;
    let expected = reqwest_013::Url::parse(expected).map_err(|_| invalid())?;
...
mod tests {
    ...
    #[test]
    fn pasted_callback_url_must_match_the_expected_loopback_endpoint() {
        let expected = "http://127.0.0.1:18766/callback";

        validate_pasted_callback_url(
            "remote",
            "http://127.0.0.1:18766/callback?code=abc&state=xyz",
            expected,
        )
        .unwrap();
        for invalid in [
            "hi",
            "http://127.0.0.1:18767/callback?code=abc&state=xyz",
            "http://127.0.0.1:18766/other?code=abc&state=xyz",
            "https://example.test/callback?code=abc&state=xyz",
        ] {
            assert!(matches!(
                validate_pasted_callback_url("remote", invalid, expected),
                Err(McpOAuthError::CallbackInput { .. })
            ));
        }
    }
    ...
    async fn file_credentials_store_round_trips_with_private_permissions() {
        ...
        store.delete_credentials_and_enrollment().await.unwrap();
        assert!(!path.exists());
    ...
    #[tokio::test]
    async fn explicit_oauth_without_credentials_fails_closed() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join(".mcp.json");
        ...
        assert!(matches!(
            result,
            Err(McpOAuthError::AuthorizationRequired { server }) if server == "docs"
        ));
    }

    #[tokio::test]
    async fn configured_client_id_mismatch_fails_closed_until_relogin() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join(".mcp.json");
        let store = McpCredentialStore::new(
            &config_path,
        ...
        let result = authorization_manager(
            ...
            McpOAuthCredentialsStore::File,
            true,
            Some("configured-client-b"),
        )
        .await;

        assert!(matches!(
            result,
            Err(McpOAuthError::ClientIdMismatch { server }) if server == "docs"
        ));
        assert_eq!(
            store.load().await.unwrap().unwrap().client_id,
            "configured-client-a"
        );
        assert!(store.is_enrolled().await.unwrap());
    }

    #[tokio::test]
    async fn enrolled_oauth_without_credentials_fails_closed_and_keeps_marker() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join(".mcp.json");
        let store = McpCredentialStore::new(
            &config_path,
        ...
        store.mark_enrolled().await.unwrap();
    ...
    #[tokio::test]
    async fn issuer_change_discards_tokens_but_keeps_enrollment_fail_closed() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        ...
        let first = authorization_manager(
            &config_path,
            "docs",
            &url,
            McpOAuthCredentialsStore::File,
        ...
        let restarted = authorization_manager(
            &config_path,
            "docs",
            &url,
            McpOAuthCredentialsStore::File,
