...
use crate::claim::{Dispute, DisputeId, DisputeStatus, ResolvedBy};
...
use super::evaluator::{
    validate_proposal, validate_verification, ArbitrationEvaluator,
    ArbitrationOutputValidationError,
};
use super::resolution::{preempt_analysis_for_resolution, ResolutionService};
...
use super::types::{
    ...
    MaintainerDisputeRecord, VerificationVerdict, ARBITRATION_PROMPT_VERSION,
    ARBITRATION_SCHEMA_VERSION, CURRENT_SEMANTIC_PROJECTION_VERSION,
};
    ...
    async fn run_proposal(
        ...
        let proposal = match proposal {
            ...
            Err(error) => {
                ...
                self.finish_failed(job, token, code, &error).await?;
                return Ok(false);
            }
        };
...
mod tests {
    ...
    use crate::maintainer::arbitration::{
        ...
        ClaimAssessmentVerification, FrozenArbitrationContext, HumanResolutionInput,
        RejectResolutionInput, ResolutionService,
    };
    ...
    async fn unresolved_outputs_reject_claim_change_advice_but_resolved_outputs_keep_it() {
        ...
        unresolved_proposal.resolution_basis = ResolutionBasis::Evidence;
        assert!(validate_proposal(context, &unresolved_proposal)
            .unwrap_err()
            .to_string()
            .contains("必须使用 insufficient_evidence basis"));
        unresolved_proposal.resolution_basis = ResolutionBasis::InsufficientEvidence;
        ...
        let mut resolved_proposal = resolved_evaluator.propose(context).await.unwrap();
        resolved_proposal.resolution_basis = ResolutionBasis::InsufficientEvidence;
        assert!(validate_proposal(context, &resolved_proposal)
            .unwrap_err()
            .to_string()
            .contains("不能使用 insufficient_evidence basis"));
        resolved_proposal.resolution_basis = ResolutionBasis::Evidence;
